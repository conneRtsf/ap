package com.example.agsale.utils;

import static com.example.agsale.utils.RxDataUtils.bytes2HexString;
import static com.example.agsale.utils.RxDataUtils.hexString2Bytes;
...
public class RxEncryptUtils {
    public static String firstChar(String originStr) {
        int pos = originStr.length() % 5;
        int position = (pos + 5) % 3; //业务需求 特意复杂着写.....
        String first = originStr.substring(position - position, position - position + 1);
        int firstNum = Integer.valueOf(str2HexStr(first));
        if (firstNum < 0 || firstNum > 15) {
            return "0";
        }
        return first;
    }
    ...
    public static String insertString(String originStr, int offset, String str) {
        StringBuffer stringBuffer = new StringBuffer(originStr);
        return stringBuffer.insert(offset, str).toString();
    }
